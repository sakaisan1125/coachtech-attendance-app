...
@php
    ...
    // UI用に最低2行は用意（空行）
    while (count($breakRows) < 2) { $breakRows[] = ['start'=>null,'end'=>null]; }
@endphp
          ...
          <tbody class="attendance-detail__table-body">
            ...
            <tr class="attendance-detail__row attendance-detail__row--inout">
              ...
              <td class="attendance-detail__value">
                ...
                @error('clock_out') <div class="form-error">{{ $message }}</div> @enderror
              </td>
            </tr>
            ...
            <tr class="attendance-detail__row attendance-detail__row--notes">
              ...
              <td class="attendance-detail__value">
                <textarea name="notes" class="attendance-detail__input-notes" rows="3">{{ old('notes', $attendance->notes) }}</textarea>
...
@if (session('error'))   <div class="msg ng">{{ session('error') }}</div> @endif
@endsection